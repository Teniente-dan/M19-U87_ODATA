CLASS zcl_zgw100_87_searchhe_dpc_ext DEFINITION
  PUBLIC
  INHERITING FROM zcl_zgw100_87_searchhe_dpc
  CREATE PUBLIC .
  ...
  PROTECTED SECTION.
    ...
    METHODS agencyset_get_entityset
        REDEFINITION .
  PRIVATE SECTION.
    DATA: gt_agencyset  TYPE zcl_zgw100_87_searchhe_mpc=>tt_agency,
          gt_countryset TYPE zcl_zgw100_87_searchhe_mpc=>tt_suptext.
ENDCLASS.



CLASS zcl_zgw100_87_searchhe_dpc_ext IMPLEMENTATION.
  ...
  METHOD agencyset_get_entityset.
    DATA: wa_country TYPE zcl_zgw100_87_searchhe_mpc=>ts_suptext.
    ...
    CASE io_tech_request_context->get_source_entity_type_name( ).
      WHEN zcl_zgw100_87_searchhe_mpc=>gc_suptext.
...
*  Source data for $expand
    DATA(lv_source_entity_set_name) = io_tech_request_context->get_source_entity_set_name( ).
    io_tech_request_context->get_converted_source_keys(
         IMPORTING
           es_key_values = wa_country
       ).
    IF wa_country IS NOT INITIAL.
      lt_filter = VALUE #( BASE lt_filter
      ( property = lv_source_entity_set_name
        select_options = VALUE #( ( sign = 'I' option = 'EQ' low = wa_country-countryid ) ) ) ).
    ENDIF.
  ...
  METHOD suptextset_get_entity.
    DATA wa_agency TYPE zcl_zgw100_87_borrrfc_mpc=>ts_agency.
*  Source data for $expand
    DATA(lv_source_entity_set_name) = io_tech_request_context->get_source_entity_set_name( ).

    io_tech_request_context->get_converted_source_keys(
         IMPORTING
           es_key_values = wa_agency
       ).
    er_entity-countryid = wa_agency-country_code.
  ...
  METHOD suptextset_create_entity.
    DATA ls_entry_data TYPE zcl_zgw100_87_searchhe_mpc=>ts_suptext.
